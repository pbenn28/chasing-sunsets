...
scoring:
  ...
  rationale:
    A: "Requires covered AI content tools to let users attach provenance information, bars large platforms from stripping or tampering with it, and bars unauthorized training on provenance-protected copyrighted works — an active process requirement backed by FTC and state-AG enforcement, not just a disclosure mandate."
    B: "The bill preserves states' ability to enforce their own civil and criminal law and creates a state-AG enforcement role, but doesn't affirmatively guarantee states can impose their own, potentially stricter, provenance requirements. Combined with the federal regulatory scheme it builds (NIST standards plus FTC/state-AG/private enforcement) and the absence of a clear savings clause, the practical effect leans toward crowding out independent state rules."
    C: "Directs NIST to develop provenance, watermarking, and detection standards, and gives the FTC and state AGs new enforcement authority over tampering and unauthorized-training violations — meaningful new authority layered onto an existing regulator rather than a wholly new oversight body."
