---
title: Disrupt Explicit Forged Images And Non-Consensual Edits Act of 2025 (DEFIANCE Act)
short_name: DEFIANCE Act
bill_numbers:
- S. 1837
- H.R. 3562
congress: 119
topic: Deepfakes & Synthetic Media
status: passed_one_chamber
chamber_origin: Both
introduced_date: '2025-05-21'
last_action: Passed the Senate by unanimous consent; received in the House and held at the desk
last_action_date: '2026-01-13'
sponsors:
- Sen. Dick Durbin (D-IL)
- Rep. Alexandria Ocasio-Cortez (D-NY)
cosponsor_count: 8
committees:
- Senate Judiciary
- House Judiciary
scoring:
  axes:
    A: 0.0
    B: 1.0
    C: 0.0
    D: 0.0
    F: 0.0
  unsigned:
    E_consumer: 5.0
  impact_components:
    R: 1.00
    D: 1.00
    E_f: 1.00
    P: 0.60
  likelihood:
    p_committee: 0.85
    p_enact: 0.30
    basis: "govtrack_corroborated"
  rationale:
    A: "Creates civil liability for people who produce, disclose, or distribute intimate digital forgeries, but places no obligations on AI developers or deployers themselves."
    B: "States and Tribal governments are explicitly free to enforce their own, even stronger protections for victims — the bill sets a floor, not a ceiling, on top of federal law. That's a savings clause for deepfake/NCII victim-protection law specifically, not a guarantee about states' authority to regulate AI development, which this bill never touches."
    C: "Enforcement runs entirely through private lawsuits in federal court; no new agency, database, or oversight body is created."
    D: "No export-control, chip-access, or geopolitical content."
    E_consumer: "Lets victims of deepfake or nonconsensual intimate imagery sue producers, distributors, and possessors nationwide, with liquidated damages of $150,000 (rising to $250,000 where tied to sexual assault, stalking, or harassment) and a 10-year statute of limitations — a broad, victim-friendly private right of action."
    F: "No data-center, permitting, interconnection, or energy content."
    R: "A federal civil remedy with no revenue floor or sector carve-out — it reaches anyone in the country who produces, discloses, or distributes a covered forgery."
    Depth: "Driven mainly by the strength of the consumer-harm remedy — this is a targeted civil-liability statute, not a broad regulatory regime."
    E_f: "Victims enforce their own rights directly in court through statutory damages, with no agency gatekeeping required."
    P: "Builds on the already-enacted TAKE IT DOWN Act's criminal/takedown framework and follows a template several states have already adopted for NCII civil remedies, rather than breaking new ground."
    likelihood: "Passed the Senate by unanimous consent in January 2026 and has strong bipartisan cosponsorship in the House (53 cosponsors split evenly across parties), leaving House floor scheduling as the main remaining hurdle rather than committee passage."
  confidence: high
  text_source: full_text
  scored_at: "2026-08-01"
tags:
- deepfakes
- NCII
- civil liability
sources:
- label: Congress.gov (Senate)
  url: https://www.congress.gov/bill/119th-congress/senate-bill/1837
summary: Senate-passed bill letting deepfake and NCII victims sue perpetrators for damages; awaits House action.
timeline:
- date: '2025-05-21'
  event: Introduced in both chambers; referred to Judiciary Committees
- date: '2026-01-13'
  event: Passed the Senate by unanimous consent; held at the House desk
---

The DEFIANCE Act creates and expands federal civil causes of action for people harmed by nonconsensual intimate imagery, including AI-generated “intimate digital forgeries.” Victims could sue those who produce, disclose, or possess-with-intent-to-distribute such images, with liquidated damages of $150,000 (or $250,000 where tied to sexual assault, stalking, or harassment) and a ten-year statute of limitations. It complements the already-enacted TAKE IT DOWN Act, which is criminal and platform-takedown focused, by adding a civil remedy.
