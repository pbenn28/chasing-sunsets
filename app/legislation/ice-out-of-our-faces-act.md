---
title: ICE Out of Our Faces Act
short_name: ICE Out of Our Faces Act
bill_numbers:
- S. 3779
congress: 119
topic: Biometrics & Surveillance
status: committee
chamber_origin: Senate
introduced_date: '2026-02-04'
last_action: Introduced in the Senate and referred to committee
last_action_date: '2026-02-04'
sponsors:
- Sen. Ed Markey (D-MA)
cosponsor_count: 4
committees:
- Senate Judiciary
scoring:
  axes:
    A: 0.0
    B: 0.0
    C: -1.0
    D: 0.0
    F: 0.0
  unsigned:
    E_consumer: 4.0
  impact_components:
    R: 0.75
    D: 0.80
    E_f: 1.00
    P: 0.80
  likelihood:
    p_committee: 0.17
    p_enact: 0.025
    basis: "base_rate_heuristic_no_live_lookup"
  rationale:
    A: "Restricts how ICE, CBP, and deputized local officers may use biometric surveillance themselves; it doesn't impose obligations on AI developers, so this dimension isn't engaged."
    B: "Sec. 3(e) explicitly preserves federal, state, and local law, stating the bill doesn't preempt or supersede them absent an actual conflict with its own limits on immigration officers. Because it only constrains federal officers' own conduct rather than states' authority over private AI developers, it has no bearing on state regulatory power."
    C: "Sec. 3(a)-(b) bars ICE and CBP, plus deputized local officers, from acquiring, possessing, accessing, or using facial recognition and other biometric surveillance, and requires deletion of data already collected. Per the frontier/systemic-risk-vs-near-term-harm guardrail, this is real government-use AI governance but narrow-domain (biometric surveillance of a specific population: immigration-enforcement targets and bystanders), not systemic AI oversight capacity — so despite textually matching the '-3, affirmatively bars an agency from collecting information or acting in the domain' rung, it's capped near the floor. Corrected from -3.0 to -1.0; the bill's real severity is already carried on E_consumer."
    D: "No export-control or geopolitical content."
    E_consumer: "Pairs a categorical ban on this surveillance (Sec. 3(a)) with a private right of action, state attorney general enforcement, actual and punitive damages, and an exclusionary rule keeping illegally obtained biometric data out of proceedings (Sec. 3(c)) -- one of the stronger enforcement packages in this batch, though the right to sue is limited to immigration-enforcement conduct."
    F: "No buildout or energy content."
    R: "Applies federally, but only to ICE, CBP, and deputized local officers rather than government-wide, narrowing its reach within that class."
    Depth: "Driven mainly by the strength of its consumer-protection provisions -- the private right of action, damages, and exclusionary rule give it real teeth beyond the underlying use ban."
    E_f: "Backed by an individual private right of action with actual and punitive damages (Sec. 3(c)(2)(B),(D)) -- the strongest enforcement mechanism available."
    P: "Combining a private right of action, an exclusionary rule, and state attorney general enforcement makes this a comprehensive model other biometric-surveillance bills are likely to draw from."
    likelihood: "Re-verified 2026-09-09: still in Senate Judiciary with no markup scheduled; cosponsor count confirmed at 4 (Wyden, Merkley, Alsobrooks, Sanders), all Democratic-caucus, no Republican co-sponsor despite the topic's bipartisan potential on civil-liberties grounds. A House companion (H.R. 7363, Jayapal) has since been introduced, which slightly raises the odds of this becoming the vehicle Congress eventually uses if ICE surveillance stays a live news story, but neither chamber shows committee movement yet."
  confidence: medium
  text_source: full_text
  scored_at: "2026-09-09"
tags:
- facial recognition
- immigration
- biometrics
- surveillance
sources:
- label: Congress.gov
  url: https://www.congress.gov/bill/119th-congress/senate-bill/3779
- label: House companion bill H.R. 7363 (Jayapal), re-verification search 2026-09-09
  url: https://www.congress.gov/bill/119th-congress/house-bill/7363
summary: Senate bill banning ICE and CBP use of facial recognition and biometric surveillance; in committee.
timeline:
- date: '2026-02-04'
  event: Introduced in the Senate and referred to committee
- date: '2026-09-09'
  event: Re-verification found no committee markup in Senate Judiciary; noted House companion bill H.R. 7363 (Jayapal) has since been introduced under the same short title
---

The bill would bar ICE and CBP officers, and certain deputized local officers, from acquiring or using facial recognition and other biometric surveillance within the U.S., require deletion of related biometric data within 30 days, make illegally obtained biometric information inadmissible in most federal proceedings, and create a private right of action plus state-AG enforcement. (Committee assignment is the best available estimate pending confirmation.)
