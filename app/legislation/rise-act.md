...
scoring:
  ...
  rationale:
    A: "Gives AI developers a civil-liability safe harbor (Sec. 4) conditioned on publicly disclosing model cards and design specs — the core effect is immunity from suit, not a disclosure mandate, even though disclosure is the price of qualifying for it. Corrected under the frontier/systemic-risk guardrail: the rubric explicitly warns that 'a narrow-domain liability shield or immunity provision shouldn't be scored as a deep negative on A' the way a general frontier-developer liability shield would be. This safe harbor only reaches AI used by licensed professionals (physicians, attorneys, engineers, financial advisors) — a narrow sectoral carve-out, not an economy-wide or frontier-developer-wide immunity. That moves it off the −3 'substantive liability limitation' rung (reserved for a general shield) down to the −2 'marginal relief: narrow carve-out' rung, mirroring how axis B already treats this same bill's preemption as narrow-domain (−2) rather than broad field preemption (−4)."
    ...
    Depth: "Recomputed after the frontier/systemic-risk guardrail correction — now driven by axis A (frontier developer stringency) at 2.0/5."
