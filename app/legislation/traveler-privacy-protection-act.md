---
title: Traveler Privacy Protection Act of 2025
short_name: Traveler Privacy Protection Act
bill_numbers:
- S. 1691
congress: 119
topic: Biometrics & Surveillance
status: committee
chamber_origin: Senate
introduced_date: '2025-05-08'
last_action: Read twice and referred to the Senate Committee on Commerce, Science, and Transportation
last_action_date: '2025-05-08'
sponsors:
- Sen. Jeff Merkley (D-OR)
- Sen. John Kennedy (R-LA)
cosponsor_count: 5
committees:
- Senate Commerce, Science, and Transportation
scoring:
  axes:
    A: 0.0
    B: 0.0
    C: 1.0
    D: 0.0
    F: 0.0
  unsigned:
    E_consumer: 2.0
  impact_components:
    R: 0.90
    D: 0.40
    E_f: 0.40
    P: 0.40
  likelihood:
    p_committee: 0.09
    p_enact: 0.04
    basis: "govtrack_corroborated"
  rationale:
    A: "Restricts TSA's own use of facial recognition on travelers rather than imposing any requirement on AI developers, so it falls outside the developer-obligation focus of this axis."
    B: "No preemption, savings-clause, or 'notwithstanding' language appears anywhere in the bill, which only amends TSA's governing statute and the Aviation and Transportation Security Act. It has nothing to say about state authority, and it creates no regulatory scheme over private AI developers."
    C: "Requires an annual GAO study and report to Congress on TSA's facial-recognition program — a real, recurring oversight mechanism."
    D: "No export-control, chip-access, or geopolitical-competition content."
    E_consumer: "Gives travelers opt-out/opt-in rights, requires affirmative express-consent notice, bars less-favorable treatment for opting out, and sets data-minimization limits — including a 24-hour cap on storing images used for identification and deletion requirements. It stops short of creating a private right of action or criminal liability."
    F: "No data-center, permitting, or energy content."
    R: "A federal bill scoped to TSA airport screening; within that narrow lane it covers essentially all checkpoints and trusted-traveler enrollment."
    Depth: "Its weight comes mostly from the traveler-protection provisions — opt-out rights, consent, and data-minimization limits — with the GAO reporting requirement adding a smaller secondary layer."
    E_f: "Enforcement relies on TSA compliance plus mandatory GAO oversight reporting; there's no stated civil penalty, no dedicated enforcement authority, and no private right of action."
    P: "Narrow and sector-specific to TSA airport screening, not built as a reusable framework for other agencies."
    likelihood: "A bipartisan pairing (Merkley and Kennedy) with five cosponsors, but the bill has sat in Senate Commerce without a committee vote since introduction in May 2025 — typical for a single-chamber bill at this stage, and its prospects remain modest."
  confidence: medium
  text_source: full_text
  scored_at: "2026-08-01"
tags:
- facial recognition
- TSA
- travel
- privacy
sources:
- label: Congress.gov
  url: https://www.congress.gov/bill/119th-congress/senate-bill/1691
summary: Bipartisan Senate bill requiring a TSA facial-recognition opt-out at airports; in committee.
timeline:
- date: '2025-05-08'
  event: Introduced and referred to Senate Commerce
---

The bill would limit TSA use of facial recognition for passenger screening — requiring TSA to offer and notify passengers of a non-facial-recognition option, barring less-favorable treatment for opt-outs, imposing data-minimization limits (no storage of facial images beyond 24 hours except for testing), and mandating GAO oversight.
