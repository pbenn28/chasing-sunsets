...
scoring:
  ...
  rationale:
    ...
    B: "Corrected a stored-value/rationale mismatch (was axes.B=0.0 despite the rationale concluding +1): the bill explicitly states it does not preempt state consumer-protection laws providing greater protections and grants state AGs parens patriae enforcement authority (paused while a federal action is pending). Under the frontier/systemic-risk-vs-near-term-consumer-harm guardrail, this savings clause is scoped to the bill's own narrow topic — AI sponsored-content/ad disclosure — not to states' authority to regulate frontier AI development generally, so it caps near the 'ordinary savings clause' rung rather than the 'affirmative guarantee with teeth' rung regardless of how unconditional the clause reads textually. Lands at +1. The implicit-preemption floor doesn't separately apply since A does not reach +2."
    ...
    Depth: "max(|A|=0.5, |B|=1.0, |C|=0, |F|=0, E=4)/5 = 0.80, driven almost entirely by the E axis."
