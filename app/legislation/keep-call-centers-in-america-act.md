---
title: Keep Call Centers in America Act of 2025
short_name: Keep Call Centers in America Act
bill_numbers:
- H.R. 4954
- S. 2495
congress: 119
topic: Civil Rights & Labor
status: committee
chamber_origin: House
introduced_date: '2025-08-12'
last_action: Referred to four House committees
last_action_date: '2025-08-12'
sponsors:
- Rep. Kristen McDonald Rivet (D-MI)
cosponsor_count: 13
committees:
- House Energy and Commerce
- House Education and Workforce
- House Oversight and Government Reform
- House Armed Services
scoring:
  axes:
    A: 1.0
    B: 0.0
    C: 0.0
    D: 0.0
    F: 0.0
  unsigned:
    E_consumer: 1.0
  impact_components:
    R: 0.50
    D: 0.20
    E_f: 0.40
    P: 0.40
  likelihood:
    p_committee: 0.05
    p_enact: 0.02
    basis: "base_rate_adjusted"
  rationale:
    A: "The only AI-specific requirement is a disclosure duty (Sec. 201(b)): businesses using AI for customer service must say so and offer to transfer the caller to a U.S.-based human agent. A disclosure obligation with no deeper substantive standard."
    B: "Contains no preemption or savings-clause language addressing state authority; the bill's one 'notwithstanding any other provision of law' clause governs federal grant/loan eligibility, not state power."
    C: "Builds a Labor Department public list of offshoring employers and bars them from certain federal grants and loans, but that's aimed at offshoring enforcement rather than governing AI systems."
    D: "No export-control or geopolitical content."
    E_consumer: "The AI piece is a plain disclosure requirement -- flag AI use, offer a human alternative -- with no criminal liability or right to sue attached."
    F: "No data-center, permitting, or energy content."
    R: "The AI-disclosure duty applies to any business nationwide using AI in customer service, but it's a secondary provision riding inside a bill mainly aimed at offshoring, not a dedicated AI measure."
    Depth: "The shallowest bill in this batch -- a disclosure-only rider with no deeper structural provisions."
    E_f: "The bill's real penalty (loss of federal grant/loan eligibility) attaches to offshoring conduct, not specifically to a failure to disclose AI use, so the AI provision itself carries no clearly stated penalty."
    P: "A narrow disclosure rule attached to a customer-service/offshoring bill, self-contained rather than part of a broader AI framework."
    likelihood: "Still sitting at Introduced with no hearing, markup, or vote, referred to four House committees at once -- a multi-committee referral that tends to slow bills down. Sponsors span both parties (Rep. McDonald Rivet, D-MI, in the House; Sen. Gallego, D-AZ, and Sen. Justice, R-WV, in the Senate) with 13 House cosponsors, a modest but real base of support."
  confidence: high
  text_source: full_text
  scored_at: "2026-08-01"
tags:
- labor
- customer service
- disclosure
sources:
- label: Congress.gov
  url: https://www.congress.gov/bill/119th-congress/house-bill/4954
summary: Requires disclosure when AI or offshore agents handle customer service, plus offshoring penalties.
timeline:
- date: '2025-08-12'
  event: Introduced in the House
- date: '2025-08-12'
  event: Referred to four House committees
---

The bill requires the Labor Department to maintain a public list of employers that offshore call-center work, bars listed firms from certain federal grants and loans, and requires agents to disclose their physical location and offer transfer to a U.S.-based agent. Relevant to AI: when a business uses AI in customer service, it must disclose that fact and inform consumers of their right to request a U.S.-based human operator.
